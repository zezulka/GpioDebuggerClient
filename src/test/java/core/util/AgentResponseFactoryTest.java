...
public class AgentResponseFactoryTest {
    ...
    public void illegalInterface() {
        assertThatThrownBy(() -> AgentResponseFactory.of(mockedAddress, "INTERFACE_UNKNOWN:" + clientPin.getPinId() + ":HIGH")).isInstanceOf(IllegalResponseException.class);
    }

    @Test
    public void responseWithExtraSpaces() {
        try {
            assertThat(AgentResponseFactory.of(mockedAddress, "GPIO : " + clientPin.getPinId() + ": HIGH ")).isInstanceOf(AgentResponse.class);
    ...
    public void happyScenarioGpioHighVoltage() {
        try {
            assertThat(AgentResponseFactory.of(mockedAddress, "GPIO:" + clientPin.getPinId() + ":HIGH")).isInstanceOf(GpioAgentResponse.class);
        } catch (IllegalResponseException ex) {
            fail(ex.getMessage());
        }
    }
    ...
    public void happyScenarioGpioLowVoltage() {
        try {
            assertThat(AgentResponseFactory.of(mockedAddress, "GPIO:" + clientPin.getPinId() + ":LOW")).isInstanceOf(GpioAgentResponse.class);
        } catch (IllegalResponseException ex) {
            fail(ex.getMessage());
        }
    }
    ...
    public void gpioInvalidVoltage() {
        assertThatThrownBy(() -> AgentResponseFactory.of(mockedAddress, "GPIO:" + clientPin.getPinId() + "MID")).isInstanceOf(IllegalResponseException.class);
    }
    ...
    public void interruptGeneratedHappyScenario() {
        try {
            ...
            assertThat(AgentResponseFactory.of(mockedAddress, "INTR_GENERATED:" + clientPin.getPinId()
                    + ':' + intrType.toString() + ':'
    ...
    public void interruptStartedHappyScenario() {
        try {
            InterruptManager.addInterruptListener(mockedAddress, new InterruptValueObject(clientPin, intrType));
            assertThat(AgentResponseFactory.of(mockedAddress, "INTR_STARTED:" + clientPin.getPinId()
                    + ':' + intrType.toString() + ':'
                    + mockedNow)).isInstanceOf(InterruptListenerAgentResponse.class);
        } catch (IllegalResponseException ex) {
            fail(ex.getMessage());
    ...
    public void interruptStoppedHappyScenario() {
        try {
            ...
            assertThat(AgentResponseFactory.of(mockedAddress, "INTR_STOPPED:" + clientPin.getPinId()
                    + ':' + intrType.toString() + ':'
                    + mockedNow)).isInstanceOf(InterruptListenerAgentResponse.class);
        } catch (IllegalResponseException ex) {
            fail(ex.getMessage());
    ...
    public void interruptGeneratedMissing() {
        assertThatThrownBy(() -> AgentResponseFactory.of(mockedAddress, "INTR_GENERATED:" + clientPin.getPinId()
                + ':' + intrType.toString() + ':'
    ...
    public void interruptStartedMissing() {
        assertThatThrownBy(() -> AgentResponseFactory.of(mockedAddress, "INTR_STARTED:" + clientPin.getPinId()
                + ':' + intrType.toString() + ':'
                + mockedNow)).isInstanceOf(IllegalResponseException.class);
    }
    ...
    public void interruptStoppedMissing() {
        assertThatThrownBy(() -> AgentResponseFactory.of(mockedAddress, "INTR_STOPPED:" + clientPin.getPinId()
                + ':' + intrType.toString() + ':'
                + mockedNow)).isInstanceOf(IllegalResponseException.class);
    }
